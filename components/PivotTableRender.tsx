...
interface FlattenedRow {
  key: string;
  dims: { label: string; rowSpan: number; depth: number }[]; 
...
const PivotTableRender: React.FC<PivotTableRenderProps> = ({ result, config, viewMode, density }) => {
  ...
  const tableRows = useMemo(() => {
    const traverse = (nodes: PivotNode[], currentDepth: number): FlattenedRow[] => {
      let rows: FlattenedRow[] = [];
      nodes.forEach(node => {
        if (node.isLeaf) {
          rows.push({
            key: node.key, 
            dims: [{ label: node.label, rowSpan: 1, depth: currentDepth }],
            values: node.values || {}
          });
        } else {
          const childRows = traverse(node.children || [], currentDepth + 1);
          if (childRows.length > 0) {
            childRows[0].dims.unshift({ label: node.label, rowSpan: childRows.length, depth: currentDepth });
          }
          rows = [...rows, ...childRows];
        }
      });
  ...
  const buildHeaderRows = () => {
           ...
           if (!node.isLeaf && node.children) {
              walk(node.children, depth + 1);
               ...
               config.values.forEach(v => {
                   const label = v.customName || `${v.aggregator.toUpperCase()} ${v.field}`;
     ...
     if (colNodes.length > 0) {
        ...
        if (showMetricHeader) {
             config.values.forEach(v => {
                 rows[0].push({ label: v.customName || `${v.aggregator.toUpperCase()} ${v.field}`, colSpan: 1, key: `total_${v.id}` });
             });
        } else if (config.values.length === 1) {
            rows[0].push({ label: config.values[0].customName || `${config.values[0].aggregator.toUpperCase()} ${config.values[0].field}`, colSpan: 1, key: `total` });
        } else {
  ...
  return (
    <div className="absolute inset-0 overflow-auto bg-white" id="pivot-container">
        <table className="w-max text-left border-collapse border-spacing-0 selectable">
                ...
                <tr key={rIdx} className="hover:bg-blue-50/30 transition-colors group">
                  {row.dims.map((dim, dIdx) => (
                    <td 
                      key={`dim-${dIdx}`} 
                      rowSpan={dim.rowSpan}
                      colSpan={dIdx === row.dims.length - 1 ? maxRowDepth - dim.depth + 1 : 1}
                      className={`
